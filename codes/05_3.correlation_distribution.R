...
#     dplyr::rename(num_exons = Exons, length = Transcript_length))
...
# guitar_combined$GS689[c("V1", "V2", "V3")] <- guitar_combined$GS689[c("V1", "V2", "V3")] %>% DRnaSeq::tpm(.,guitar_combined$GS689$length)
# guitar_combined$PC3E[c("V1", "V2", "V3")] <- guitar_combined$PC3E[c("V1", "V2", "V3")] %>% DRnaSeq::tpm(.,guitar_combined$PC3E$length)
#####################
# 04. three_gen
#####################
...
get_distribution <- function(stringtie_tx, stringtie_impute, guitar_combined, three_gen) {
    ...
    stringtie_tx <- stringtie_tx %>%
        select(starts_with("V")) %>%
        pivot_longer(cols = everything(), names_to = "sample", values_to = "TPM") %>%
        remove_outliers(., "TPM") %>%
        # mutate(TPM = log(TPM + 1)) %>%
        mutate(method = "stringtie_tx")
    stringtie_impute <- stringtie_impute %>%
        select(starts_with("V")) %>%
        pivot_longer(cols = everything(), names_to = "sample", values_to = "TPM") %>%
        remove_outliers(., "TPM") %>%
        # mutate(TPM = log(TPM + 1)) %>%
        mutate(method = "stringtie_impute")
    guitar_combined <- guitar_combined %>%
        select(starts_with("V")) %>%
        pivot_longer(cols = everything(), names_to = "sample", values_to = "TPM") %>%
        remove_outliers(., "TPM") %>%
        # mutate(TPM = log(TPM + 1)) %>%
        mutate(method = "guitar_combined")

    # write min-max normalization function, and apply it to three_gen$TPM
    three_gen <- three_gen %>%
        pivot_longer(cols = everything(), names_to = "sample", values_to = "TPM") %>%
        remove_outliers(., "TPM") %>%
        # mutate(TPM = log(TPM + 1)) %>%
        mutate(method = "three_gen")
    # plot the distribution of each method
    # browser()
    distribution <- bind_rows(stringtie_tx, stringtie_impute, guitar_combined, three_gen) %>%
        # mutate(TPM = rescale(TPM, to = c(0, 1))) %>%
        # use log scale
        # mutate(TPM = log(TPM + 1)) %>%
        ggplot(aes(x = TPM, fill = method)) +
        geom_density(alpha = 0.5) +
        # facet by method
        facet_wrap(~method, scales = "free")
    # limit x axis to 0-0.05
    # scale_x_continuous(limits = c(0, 0.01))
}

GS689_dist <- get_distribution(stringtie_tx$GS689, stringtie_impute$GS689, guitar_combined$GS689, three_gen %>% select(starts_with("GS689")))
ggsave("data/05.correlations/distribution/GS689_dist.png", GS689_dist, width = 10, height = 10)
PC3E_dist <- get_distribution(stringtie_tx$PC3E, stringtie_impute$PC3E, guitar_combined$PC3E, three_gen %>% select(starts_with("PC3E")))
ggsave("data/05.correlations/distribution/PC3E_dist.png", PC3E_dist, width = 10, height = 10)
