...
# 05. Venn
#####################
# create a list of dataframes for each methods, containing Transcript_ID
venn_list <- list(
    stringtie = stringtie_tx,
    stringtie_impute = stringtie_impute,
    guitar = guitar_combined,
    three_gen = three_gen_list
) %>%
    map(~ map(.x, ~ select(.x, Transcript_ID))) %>%
    list_transpose()

# use ggVennDiagram to plot Venn diagram separately for each GS689 and PC3E
p <- list()
p$GS689 <- ggVennDiagram(venn_list$GS689 %>% map(~ pull(.x, Transcript_ID)))
p$PC3E <- ggVennDiagram(venn_list$PC3E %>% map(~ pull(.x, Transcript_ID)))

# save the plot
ggsave("data/06.venn/venn_GS689.png", p$GS689, width = 10, height = 10)
ggsave("data/06.venn/venn_PC3E.png", p$PC3E, width = 10, height = 10)

# upset plot
png("data/06.venn/upset_GS689.png", width = 10, height = 10, units = "in", res = 300)
upset(fromList(venn_list$GS689 %>% map(~ pull(.x, Transcript_ID) %>% as.character())), , order.by = "freq", sets = c("stringtie", "stringtie_impute", "guitar", "three_gen"), keep.order = TRUE)
dev.off()
png("data/06.venn/upset_PC3E.png", width = 10, height = 10, units = "in", res = 300)
upset(fromList(venn_list$PC3E %>% map(~ pull(.x, Transcript_ID) %>% as.character())), , order.by = "freq", sets = c("stringtie", "stringtie_impute", "guitar", "three_gen"), keep.order = TRUE)
dev.off()
